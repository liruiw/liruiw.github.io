...
test("serves browser app code that captures the microphone and connects over WebRTC", async () => {
  await withServer({ env: { OPENAI_API_KEY: "sk-test" } }, async (baseUrl) => {
...
test("OPTIONS /session returns CORS headers", async () => {
  await withServer({ env: { OPENAI_API_KEY: "sk-test" } }, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/session`, { method: "OPTIONS" });
